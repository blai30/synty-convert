...
```
synty_convert.py      CLI entry point: argument parsing and wiring
conversion.py         discovery, per-pack context, the pool of Blender processes
manifests.py          material records and the flavor siblings generated from them
reporting.py          every console report
...
material_flavors.py   reads the curated flavor sets and companion maps
material_names.py     decides what a converted material is called
fbx_ascii.py          reads ASCII FBX, which Blender's importer cannot
split_character_head.py  separates a character's head onto its own mesh
...
Blender embeds a CPython interpreter and reaches its data model only through it, so `bpy`, `bmesh` and `mathutils` are importable from nothing else. There is no ABI to bind against from another language, because the interpreter lives inside the process rather than beside it. The worker therefore gets no choice of language at all: picking a different one is picking not to use Blender.

That is the decision actually on the table, since the alternative is a parser like `ufbx` or the FBX SDK writing glTF directly. Blender is here for what it already knows how to do: import an FBX with its custom split normals intact, hold an armature whose rest pose can be baked and whose curves can be rescaled, expose a material as a node graph with named sockets, weld a mesh and walk its islands. Every one of those is load bearing somewhere in this document, and every one is a place where a subtle mistake renders as plausible but incorrect art.

The CLI is a free choice and stays Python anyway. `texture_matching.py` runs on both sides of the process boundary, indexing a pack in the CLI and resolving against that index in the worker, so a second language would mean a second implementation of the same scoring heuristic obliged to agree with the first exactly and forever. Blender bundles the one thing the worker needs beyond the standard library, `numpy`, so neither side installs anything.
...
The same read gets the rest of the material for free, so every property the FBX declares now survives. What a channel is *for* comes from the socket; what it *points at* still goes through the same matcher, the same overrides and the same report, so an emissive map that fails to resolve is as visible as an albedo that does.
...
A map the atlas itself declares is the one thing excluded, because it is not such a property: companions are keyed per atlas, so every material wearing that atlas wears the same map, and naming it only repeats the base name. The packs say the same thing in their own filenames. FantasyKingdom ships `Wall_Brick_01.png` beside `Wall_Brick_01_Normals.png`, all five walls pairing exactly; Military ships thirteen base names with both a `_D` and an `_N`; Dungeons Realms ships one `Dungeons_2_Texture_Normal_Map.png` for the entire pack. The convention is `<base>_<channel marker>`, so Synty has already factored the material name out. Qualifying on a companion inverts that: it named 1032 Dungeons Realms materials after a file the whole pack shares, and 347 of the 350 maps in the output carried a suffix that discriminated nothing.

What decides the name is the map a channel settled on and never the reference that found it, so `map_qualifier` compares the channel's `member` against the target declared for that atlas. A material naming the very file its atlas declares therefore answers the same as one the table filled, while its reference and match stay on the record for the report to review. A channel that resolved to nothing has no `member` and so always qualifies, which is correct: FantasyKingdom's four materials naming a normal the pack never shipped go without the map the rest of their atlas wears.

An emission color never qualifies alongside a map, because `build_material` discards a declared color the moment a map covers it. Naming one would split a material off from atlas-mates it renders identically to.

Qualifiers are derived from the material and never from collision order, which matters because names are assigned per file inside Blender while the manifest is merged across the whole pack afterwards. A suffix handed out because two materials happened to meet in one file would mean something different in the next file.
...
**Tokens shared by most of the pack cannot carry a match alone.** `texture`, `01` and `a` say nothing when most textures contain them. Without this rule `Dungeons_Texture` captured `Track_Texture` on the strength of the word `texture`.
...
The mappings stop where the evidence does. A reference to a pack you do not own has no counterpart to find, and most normal map references name a file the pack never shipped. Ambiguity is a separate case with its own answer: where a pack ships several equally plausible candidates for one surface, `material_overrides.json` groups them into a flavor set, binds the material to a declared default and ships every candidate as a material the consumer can swap to. A wrong texture renders as plausible but incorrect art, which is much harder to spot than an untextured model that the report names explicitly.
